...
<html>
	...
	<body>
		<div class="container">			
			...
			<form action=<c:if test="${type eq '整车'}">"/deal/purchase.html"</c:if>
						<c:if test="${type eq '配件'}">"/deal/ppurchase.html"</c:if> method="post">
				<s:token/>
				
				<div class="chithead"> 
					<p class="title">${type}进货单</p>					
					<div class="right">
				...
				<table class="chitbody" cellspacing="4">
					...
					<c:if test="${type eq '整车'}"><td>条码区：<span class="errors">${errors['voucher.codeback'][0]}</span></td></c:if> </tr>
				<tr><td>						
						<p>供货单位</p>												
						...
						<input type="submit" name="method:unitList" value="选择供货单位" <c:if test="${checked}">disabled</c:if> />	
						<span class="errors">${errors['voucher.dealingUnit'][0]}</span>
						<span class="errors">${errors['voucher.subAccount'][0]}</span>
					</td>
						<c:if test="${type eq '整车'}"><td rowspan="4"><textarea name="voucher.productBarcodeTxt" rows="6" class="short_input" <c:if test="${checked}">disabled</c:if>>${voucher.productBarcodeTxt}</textarea></td></c:if>
					</tr>						
					<tr><td>	
						<p>经手人</p>
					...
					<tr><td>
						...
						<input type="submit" name="method:warehouseList" value="选择收货仓库" <c:if test="${checked}">disabled</c:if> />
						<span class="errors">${errors['voucher.warehouse'][0]}</span>
					</td></tr>
					<tr><td>
						<p>发货通知单编码</p>						
						<input type="text" name="voucher.shipOrderCode" value="${voucher.shipOrderCode}" class="short_input" readonly <c:if test="${checked}">disabled</c:if>/>
						<input type="submit" <c:if test="${type eq '整车'}"> name="method:shipOrderList" </c:if>
										<c:if test="${type eq '配件'}"> name="method:partShipOrderList" </c:if>  value="选择发货通知单" <c:if test="${checked}">disabled</c:if> />
						<span class="errors">${errors['voucher.shipOrderCode'][0]}</span>
					</td></tr>
					<tr><td>	
						<p>摘要</p><input type="text" name="voucher.brief" value="${voucher.brief}" class="long_input" <c:if test="${checked}">disabled</c:if>/>
					</td><c:if test="${type eq '整车'}"><td><input type="submit" name="method:entercode" value="确认条码" <c:if test="${checked}">disabled</c:if>/></td></c:if> </tr>
					<tr><td>	
				...
				<table class="noprint">
					...
					<input type="submit" name="method:importProduct" value="导入商品" <c:if test="${errors['voucher.product'][0] == null}">disabled</c:if> />
					<br/>
					<span class="errors">${errors['voucher.product'][0]}</span>
					...
					<tbody>					
						<c:forEach var="voucherDetail" items="${voucherDetailList}" varStatus="num">
							...
							<tr <c:if test="${num.count%2 eq 0 }">bgcolor="#f8f8ff"</c:if> >
								...
								<input type="checkbox" name="selectList" value="${num.index}"  <c:if test="${checked}">disabled</c:if>>									
								...
								<td nowrap>
									<input type="text" name="voucherDetailList[${num.index}].discount" value="${voucherDetail.discount}" <c:if test="${checked}">disabled</c:if>>
								</td>
								<td nowrap>
						...
						<tr>
							...
							<td>${voucher.qty}</td>
							<td><c:if test="${viewPrice}">合计金额</c:if></td>
							<td><c:if test="${viewPrice}"><fmt:formatNumber value="${voucher.total}" pattern='#,##0.00'/></c:if></td>
						</tr>
					</tbody>
				</table>
				...
				<table>
					<tr>
						...
						<td align="right">
							优惠金额
							<input name="voucher.discount" value="<fmt:formatNumber value='${voucher.discount}' pattern='#,##0.00'/>" class="short_input" <c:if test="${checked}">disabled</c:if>/>
							&nbsp;&nbsp;&nbsp;&nbsp;
							实际金额
							<input name="voucher.actualTotal" value="<fmt:formatNumber value='${voucher.actualTotal}' pattern='#,##0.00'/>" class="short_input" readonly <c:if test="${checked}">disabled</c:if>/>
						</td>
						</c:if>
					</tr>
					<tr >
						...
						<td align="right">							
							付款账户
							<input type="hidden" name="voucher.subAccountId" value="${voucher.subAccountId}"/>
							<input name="voucher.subAccountType" value="${voucher.subAccountType}" class="short_input" readonly <c:if test="${checked}">disabled</c:if>/>
						<c:if test="${viewPrice}">
							&nbsp;&nbsp;&nbsp;&nbsp;						
							付款金额 <input type="text" name="voucher.paidMoney" value="<fmt:formatNumber value='${voucher.paidMoney}' pattern='#,##0.00'/>" class="short_input" <c:if test="${checked}">disabled</c:if>/>					
						</c:if>
						</td>
					</tr>
					<tr>
						...
						<td align="right" class="noprint">							
							...
							<input type="submit" name="method:check" value="记账" <c:if test="${checked || (! check)}">disabled</c:if> />							
