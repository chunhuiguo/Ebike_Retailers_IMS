...
<html>
	<head>
		...
		<script type="text/javascript" src="/js/deal/sale.js"></script>
		...
		$('#print').click(function(){
	        ...
	        LODOP.ADD_PRINT_TEXT(3, 32, 146, 25, "绿源电动车销售单");
	        ...
	        LODOP.ADD_PRINT_TEXT(21, 50, 30, 20, "单价");
	        LODOP.SET_PRINT_STYLEA(0, "FontSize", 6);
	        LODOP.ADD_PRINT_TEXT(21, 110, 25, 20, "数量");
	        LODOP.SET_PRINT_STYLEA(0, "FontSize", 6);
	        LODOP.ADD_PRINT_TEXT(21, 130, 30, 20, "小计");
	        LODOP.SET_PRINT_STYLEA(0, "FontSize", 6);
			var gaodu = 36;
	        $(".trc").each(function(i){
	            ...
	            LODOP.ADD_PRINT_TEXT(gaodu+12, 50, 60, 20, $(this).children("td:eq(7)").children("input").val());
	            LODOP.ADD_PRINT_TEXT(gaodu+12, 110, 20, 20, $(this).children("td:eq(6)").children("input").val());
	            LODOP.ADD_PRINT_TEXT(gaodu+12, 130, 60, 20, $(this).children("td:eq(9)").children("input").val());
	            gaodu += 30;
	        	 });

	        LODOP.ADD_PRINT_RECT(gaodu, -1, 210, 1, 0, 1);
	        gaodu += 5;
	        LODOP.ADD_PRINT_TEXT(gaodu, 10, 180, 5, "合计金额："+($("#actualTotal").val()/1+$("#discount").val()/1)+"元");
	        gaodu += 15;
	        LODOP.ADD_PRINT_TEXT(gaodu, 10, 180, 5, "收款金额："+$("#actualTotal").val()+"元");
	        gaodu += 15;
	        LODOP.ADD_PRINT_TEXT(gaodu, 10, 180, 5, "优惠金额："+$("#discount").val()+"元");
	        gaodu += 15;
	        LODOP.ADD_PRINT_TEXT(gaodu, 10, 180, 5, "单据编码："+$("#vouchercode").val());
	        LODOP.PRINT();
	        LODOP.ADD_PRINT_TEXT(gaodu, 10, 180, 5, "购买日期："+$("#date").val());
	        LODOP.PRINT();
			return false;
		});
		});
	...
	<object id="LODOP" classid="clsid:2105C259-1E0C-4534-8141-A753534CB4CA" width="0" height="0"> 
		...
		<div class="container">			
			...
			<form action=<c:if test="${type eq '整车'}">"/deal/sale.html"</c:if>
						<c:if test="${type eq '配件'}">"/deal/psale.html"</c:if> method="post">
				<s:token/>
				
				<div class="chithead"> 
					<p class="title">${type}销售单</p>					
					<div class="right">
				...
				<table class="chitbody" cellspacing="4">
					...
					<c:if test="${type eq '整车'}"><td>条码区：<span class="errors">${errors['voucher.codeback'][0]}</span></td></c:if>
					</tr>
					<tr><td>						
						<p>购买单位</p>												
						...
						<input type="submit" name="method:unitList" value="选择购买单位" <c:if test="${checked}">disabled</c:if> />	
						<span class="errors">${errors['voucher.dealingUnit'][0]}</span>
						<span class="errors">${errors['voucher.subAccount'][0]}</span>
					</td>
					<c:if test="${type eq '整车'}"><td rowspan="4"><textarea name="voucher.productBarcodeTxt" rows="6" class="short_input" <c:if test="${checked}">disabled</c:if>>${voucher.productBarcodeTxt}</textarea></td></c:if> </tr>
					...
					<c:if test="${type eq '整车'}"><td><input type="submit" name="method:entercode" value="确认条码" <c:if test="${checked}">disabled</c:if>/></td></c:if> </tr>
				</table>
				...
				<table class="chitdetail" >
					...
					<tbody>					
						<c:forEach var="voucherDetail" items="${voucherDetailList}" varStatus="num">
							...
							<tr <c:if test="${num.count%2 eq 0 }">bgcolor="#f8f8ff"</c:if> class="trc">
				...
				<table>
					<tr>
						...
						<td align="right">
							优惠金额
							<input id="discount" name="voucher.discount" value="<fmt:formatNumber value='${voucher.discount}' pattern='#,##0.00'/>" class="short_input" <c:if test="${checked}">disabled</c:if>/>
							&nbsp;&nbsp;&nbsp;&nbsp;
							实际金额
							<input id="actualTotal" name="voucher.actualTotal" value="<fmt:formatNumber value='${voucher.actualTotal}' pattern='#,##0.00'/>" class="short_input" readonly <c:if test="${checked}">disabled</c:if>/>
						</td>
					</tr>
					<tr >
						<td></td>
						<td align="right">							
							...
							&nbsp;&nbsp;&nbsp;&nbsp;						
							收款金额 <input type="text" name="voucher.paidMoney" value="<fmt:formatNumber value='${voucher.paidMoney}' pattern='#,##0.00'/>" class="short_input" <c:if test="${checked}">disabled</c:if>/>					
						</td>
					</tr>
					<tr>
						<td></td>
