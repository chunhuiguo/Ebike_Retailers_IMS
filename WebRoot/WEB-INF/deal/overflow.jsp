...
<html>
	<head>
		...
		<script type="text/javascript" src="/js/deal/overflow.js"></script>
	...
	<body>
		<div class="container">			
			...
			<form action=<c:if test="${type eq '整车'}">"/deal/overflow.html"</c:if>
						<c:if test="${type eq '配件'}">"/deal/poverflow.html"</c:if> method="post">
				<s:token/>
				
				<div class="chithead"> 
					<p class="title">${type}报溢单</p>					
					<div class="right">
				...
				<table class="chitbody" cellspacing="4">
					...
					<tr><td>
						...
						<input type="submit" name="method:warehouseList" value="选择入库仓库" <c:if test="${checked}">disabled</c:if> />
