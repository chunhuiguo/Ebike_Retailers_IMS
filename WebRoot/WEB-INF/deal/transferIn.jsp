...
<html>
	<head>
		...
		<script type="text/javascript" src="/js/deal/transferIn.js"></script>
	...
	<body>
		<div class="container">			
			...
			<form action=<c:if test="${type eq '整车'}">"/deal/transferIn.html"</c:if>
						<c:if test="${type eq '配件'}">"/deal/ptransferIn.html"</c:if> method="post">
				<s:token/>
				
				<div class="chithead"> 
					<p class="title">${type}调拨入库单</p>					
					<div class="right">
				...
				<table class="chitbody" cellspacing="4">
					...
					<tr><td>						
						...
						<input type="submit" name="method:unitList" value="选择发货单位" <c:if test="${checked}">disabled</c:if> />	
					...
					<c:if test="${type eq '整车'}"><td><input type="submit" name="method:entercode" value="确认条码" <c:if test="${checked}">disabled</c:if>/></td></c:if>
					</tr>
				</table>
