...
<body>	 
<div class="container">    	  
	  <form action="/info/priceQueryList.html" method="post">
	  <s:token/>
	    <div class="chithead"> 
	    <table>
	    <tr>
	    <td>
	    <p class="title">整车销售价格查询</p>
	    </td>						
		</tr>
		<tr>
		<td><p class="title"> 整车编码：${product.code} &nbsp;整车名称：${product.prefixName}${product.suffixName}&nbsp;整车颜色：${product.colorName}</p>
		</td>	
		<td><input type="submit" id="submit" name="submit" value="选择整车" />
		</td>	
		</tr>
		</table>					
	    </div>		
					...
					<tbody>
						<c:forEach var="price" items="${priceList}" varStatus="num">
							...
							<tr <c:if test="${num.count%2 eq 0 }">bgcolor="#f8f8ff"</c:if> >						
								...
								<td nowrap>
								<fmt:formatNumber value="${price.price}" pattern="#,##0.00"/>	<!-- <input type="text" name="price.price" value="<fmt:formatNumber value='${price.price}' pattern='#,##0.00'/>" readonly> -->									
								</td>																							
						    </tr>
						</c:forEach>						
					</tbody>
